...
Lights live in the ECS. There are three components — a directional sun, point lights, and spot
lights — and nothing is baked. Each frame the lights are gathered from the scene, packed into
GPU structs, and uploaded into the lighting set the fragment shader reads.
...
and an inverse-square falloff with a hard `range`. A spot adds a cone aimed by `direction`,
with a soft edge between `innerAngle` and `outerAngle`.

## Two GPU shapes

The directional light and the punctual lights take different paths to the GPU because they are
evaluated differently. The directional light is a handful of scalars folded straight into the
lighting UBO (set 1, binding 0). The punctual lights become a variable-length array, one
`GpuLight` per point or spot light:
...
Four `vec4`s keeps the struct naturally aligned for std430. A point light leaves
`directionType` zeroed; a spot writes its normalized direction with type `1` in `.w` and
pre-computes the cosines of its two half-angles into `spotCos`. The shader compares against
cosines, so converting degrees → cosine once on the CPU is cheaper than a `cos` per fragment.
`renderScene` builds the array with two `forEach` loops over the point and spot components.

## The upload

`setSceneLighting` writes the current frame's copies. It is safe to write directly because
