...
namespace se
{
    ...
    // GPU-profiler / frame-timing internals, defined in renderer_profiler.cpp. Visible here for
    // the frame loop (beginFrame / endFrame) and destroyRenderer; the public profiler/perf/alarm
    // entry points are declared in the :Types partition.
    ...
    auto newRenderer(Window& window) -> Result<Renderer>
    {
        ...
        // downstream never draws hardware conclusions.
        {
            ...
            if (validBits >= 64)
            {
                renderer.profiler.timestampMask = ~0ULL;
            }
            else
            {
                renderer.profiler.timestampMask = (1ULL << validBits) - 1ULL;
            }
        ...
        if (hasCalibratedTs && std::getenv("SAFFRON_DISABLE_CALIBRATION") == nullptr)
        {
            ...
            if (renderer.profiler.calibration.available)
            {
                logInfo("calibrated timestamps available — GPU spans correlate to the CPU clock");
            }
            else
            {
                logInfo("calibrated timestamps unavailable — GPU spans stay on their own axis");
            }
        }
    ...
    // Free a view's per-frame GPU images. Shared by destroyRenderer and destroyView so a new
    // per-view image is released in exactly one place; adding one to only one path would leak.
    void resetViewImages(ViewTargets& vt)
    {
        vt.offscreen.reset();
        vt.depth.reset();
        vt.gNormal.reset();
    ...
    void resetRestirView(RestirView& rv)
    {
        rv.radiance.reset();
        rv.initial.reset();
        rv.combined.reset();
    ...
    {
        if (renderer.profiler.subScopes)
        {
            return &renderer.profiler.scopeRecorder;
        }
        return nullptr;
    }
    ...
    void beginFrameGraph(Renderer& renderer)
    {
        ...
        // Load the pre-pass depth when a depth pre-pass ran; otherwise clear it in this pass.
        ...
        if (renderer.graph.hasSsgi)
        {
            ...
            // sampler read + this frame's seed agree. A barrier-only pass that declares a final
            // compute SampledRead lets the graph emit the General -> ShaderReadOnly transition.
            RgPass restorePass;
    ...
    void endFrame(Renderer& renderer)
    {
        ...
        if (renderer.context.debugLabels.begin != nullptr)
        {
            labels = &renderer.context.debugLabels;
        }
        ...
        if (renderer.profiler.mode != ProfilerMode::Off && renderer.profiler.poolsReady)
        {
            ...
            if (statsOn)
            {
                sr.statsPool = renderer.profiler.statsPools[renderer.frame.index];
                sr.statsCapacity = MaxProfiledScopes;
            }
            else
            {
                sr.statsPool = vk::QueryPool{};
                sr.statsCapacity = 0;
            }
            sr.nextStatsSlot = 0;
            tsPtr = &sr;
        ...
        if (renderer.lastFrameNs != 0)
        {
            ...
            if (renderer.frameMs == 0.0f)
            {
                renderer.frameMs = deltaMs;
            }
            else
            {
                renderer.frameMs = renderer.frameMs * 0.9f + deltaMs * 0.1f;
            }
        }
        ...
        u64 frameSpanNs = 0;
        if (nowNs > renderer.frameCpuStartNs)
        {
            frameSpanNs = nowNs - renderer.frameCpuStartNs;
        }
        u64 busyNs = 0;
        if (frameSpanNs > renderer.frameWaitNs)
        {
            busyNs = frameSpanNs - renderer.frameWaitNs;
        }
        const f32 busyMs = static_cast<f32>(busyNs) / 1.0e6f;
        const f32 waitMs = static_cast<f32>(renderer.frameWaitNs) / 1.0e6f;
        if (renderer.cpuFrameMs == 0.0f)
        {
            renderer.cpuFrameMs = busyMs;
        }
        else
        {
            renderer.cpuFrameMs = renderer.cpuFrameMs * 0.9f + busyMs * 0.1f;
        }
        if (renderer.cpuWaitMs == 0.0f)
        {
            renderer.cpuWaitMs = waitMs;
        }
        else
        {
            renderer.cpuWaitMs = renderer.cpuWaitMs * 0.9f + waitMs * 0.1f;
        }
    ...
    auto renderStats(const Renderer& renderer) -> RenderStats
    {
        ...
        if (renderer.frameMs > 0.0f)
        {
            stats.fps = 1000.0f / renderer.frameMs;
        }
        else
        {
            stats.fps = 0.0f;
