# Phase 11 — the timeline in the rig editor

**Status:** NOT STARTED

## Goal

Mount the shared timeline (phase 10) in the rig-editor workspace's bottom strip, targeted at the
...
- `TimelineTransport` + `TimelineSurface` with the `TimelineTarget` seam (phase 10).
- The state source: entering the preview selects the rig (phase 4) **and the selection stays on the
  rig** — phase 8's bone highlighting deliberately uses the highlight channel, not scene selection,
  ...
  rig, source the state keyed to `rigEntity` instead — but with phase 8 as specified, the
...
- Clip switching comes from the clip list (phase 9) — the transport's clip `Select` stays hidden
  here.
...
`rigEntity` comes from the enter-result (phase 4 — the spawned rig entity uuid, not the mesh asset
uuid that keys the tab); `enabled` keys off the tab being active and the preview being entered
(error/migration states render the empty surface).
...
- The footer reads `Duration · 1 track · N clips` from the rig's linked clips (phase 9's count),
...
- Manual (`make run`): open the leg rig → press play: the leg bends in the live preview while the
  ...
  back/forward; Space toggles; switch clips via the list (phase 9) and the lane re-lays.
