# Phase 8 — the skeleton tree panel
...
skeleton overlay's selected highlight + the Inspector-style details slot in phase 9). UE5's Skeleton
Tree is the reference; per-bone retarget/socket columns are not copied.

## What exists to build on

- `get-rig` (phase 2) returns the flat parent-indexed bone list
...
- Bone → preview entity table: `enter-rig-preview`'s result (phase 4) maps each `get-rig` bone
  index to its spawned preview-scene entity uuid (`RigBoneEntityDto`) — the table this panel
  highlights through. (The engine-side table lives in phase 4 with the command + its fixture; this
  panel just consumes it.)
- **The highlight channel (phase 5), not scene selection.** `buildSkeletonOverlay` (rewritten in
  phase 5) keys off the *previewed rig*, not `editor.selected`, and carries a highlighted-joint
  ...
  has no `AnimationPlayerComponent`) and break the rig timeline (phase 11), and the old overlay
  bailed unless the selection carried `SkinnedMeshComponent`. Highlighting via the dedicated channel
...
locally in the component). Filter out the phase-5 furnishing by construction (the tree renders
`get-rig` data, not scene entities — the floor never appears).

### 2. Highlight wiring

Bone row click → `set-skeleton-highlight { joint: boneIndex }` (phase 5) through a coalescer (rapid
...
is the deliberate split that keeps the rig timeline (phase 11) fed: the engine selection stays on
the rig the whole time.

### 3. Toolbar hooks

The workspace toolbar (phase 7) gains the bone-related toggles next to where they conceptually
...
- `make e2e`: `set-skeleton-highlight` on a joint index → the highlighted-joint screenshot differs
  ...
  this panel rides land in phases 4 (bone table) and 5 (highlight channel), so this phase adds no
...
- **The tree never touches scene selection** — that is the whole point of the highlight channel.
  ...
  `animationState` slice the timeline reads never goes null (the bug phase 11 would otherwise hit).
